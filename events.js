...
// Nightlife Energy Colors for the six events
const energyColors = [
    'rgba(212, 175, 55, 0.20)',  // Tuesday - Munhumutapa Day: Regal Gold & Bronze
    'rgba(200, 255, 100, 0.18)', // Wednesday - DIVA DAYOUT: Electric Lime / Cocktail Glow
    'rgba(230, 50, 255, 0.18)',  // Thursday - Karaoke and Chaos: Vivid Magenta / Neon Pulse
    'rgba(255, 50, 100, 0.20)',  // Friday - Aphrodisiac Fridays: Sensual Crimson / Rose Red
    'rgba(255, 140, 0, 0.18)',   // Saturday - Baddies & Bottles: Amber Whiskey / Gold VIP
    'rgba(180, 70, 255, 0.18)'   // Sunday - Felt Naughty by Sunday: Midnight Velvet / Purple
];

// Emotional Energy Profiles for each poster
const energyProfiles = [
    { // Tuesday - Munhumutapa Day: Regal, Grounded, Majestic Rhythm
        driftSpeed: 0.15,
        pulseFreq: 1.0,
        parallaxMult: 0.9,
        glowBase: 0.95,
    ...
    { // Wednesday - DIVA DAYOUT: Smooth, Seductive, Lush
        driftSpeed: 0.12,
        pulseFreq: 0.8,
        parallaxMult: 0.8,
        glowBase: 0.96,
    ...
    { // Thursday - Karaoke and Chaos: Energetic, Dynamic, Nightlife Rhythm
        driftSpeed: 0.32,
        pulseFreq: 2.6,
        parallaxMult: 1.3,
        glowBase: 0.90,
    ...
    { // Friday - Aphrodisiac Fridays: Seductive, Warm, Heartbeat Pulse
        driftSpeed: 0.18,
        pulseFreq: 1.4,
        parallaxMult: 1.0,
        glowBase: 0.94,
    ...
    { // Saturday - Baddies & Bottles: Dominant, Heavy, VIP Energy
        driftSpeed: 0.22,
        pulseFreq: 1.8,
        parallaxMult: 1.2,
        glowBase: 0.97,
    ...
    { // Sunday - Felt Naughty by Sunday: Atmospheric, Late-Night, Sensual Drift
        driftSpeed: 0.14,
        pulseFreq: 0.9,
        parallaxMult: 0.85,
        glowBase: 0.95,
...
/**
 * Chronological & Weekly Recursive Countdown Engine
 * Calculates time remaining until the scheduled calendar date or next recurring night
 */
function getNextOccurrence(dayOfWeek, hour, specificDateStr) {
    const now = new Date();
    
    // If the calendar date is upcoming, count down directly to that event date
    if (specificDateStr) {
        const specificTarget = new Date(`${specificDateStr}T${hour.toString().padStart(2, '0')}:00:00`);
        if (specificTarget.getTime() > now.getTime()) {
            return specificTarget;
        }
    }
    
    // Otherwise calculate next recurring day of the week
...
const eventSchedule = [
    {
        ...
        hour: 18,
        theme: "RnB Hyms and Tonic Tuesdays",
        performers: "DJ Deno",
    ...
    },
    {
        ...
        day: 3, // Wednesday
        date: "2026-09-16",
        dateDisplay: "Wednesday, 16 September 2026",
        hour: 18,
        performers: "DJ Appollo and DJ Deno",
        offer: "Buy 1 Cocktail and Get 1 Free",
    ...
    },
    {
        ...
        hour: 20,
        performers: "DJ Butler",
        host: "Miss Andy — The Karaoke Queen",
        offer: "Best Karaoke Performance Wins a Wine Bottle",
    ...
    },
    {
        ...
        hour: 20,
        performers: "DJ Luvex and DJ Youngvibez",
        mc: "MC Mocla",
    ...
    {
        name: "BADDIES & BOTTLES SATURDAYS",
        day: 6, // Saturday
        date: "2026-09-19",
        dateDisplay: "Saturday, 19 September 2026",
        hour: 21,
        performers: "DJ Butler & DJ Timz",
        mc: "MC Wish",
        offer: "Jack Daniels Single Barrel Select on Promo — $75",
...
function updateCountdown() {
    ...
    const targetDate = getNextOccurrence(currentEvent.day, currentEvent.hour, currentEvent.date);
    ...
    const titleBadge = document.getElementById('countdown-event-title');
    const label = document.getElementById('countdown-label');
    const display = document.getElementById('timer-string');

    if (titleBadge) {
        titleBadge.innerText = currentEvent.name;
    }
    if (label && display) {
...
// Background Poster Preloader for WebP Performance
function preloadPosters() {
    eventSchedule.forEach((evt, idx) => {
        if (idx > 0 && evt.posterUrl) {
            const img = new Image();
            img.src = evt.posterUrl;
        }
    });
}
...
function initBillboard() {
    // Ensure state references all current DOM nodes
    billboardState.slides = document.querySelectorAll('.event-slide');
    billboardState.navDots = document.querySelectorAll('.nav-dot');
    billboardState.container = document.querySelector('.billboard-container');
    ...
    // Touch Swipe Gesture Support for mobile experience
    let touchStartX = 0;
    let touchStartY = 0;
    ...
    document.addEventListener('touchend', (e) => {
        ...
        // If horizontal swipe detected
        if (Math.abs(diffX) > 45 && Math.abs(diffY) < 70) {
            clearInterval(billboardState.interval);
            if (diffX < 0) {
                nextSlide();
            } else {
                let prev = (billboardState.currentIndex - 1 + billboardState.slides.length) % billboardState.slides.length;
                switchSlide(prev);
            }
            billboardState.interval = setInterval(nextSlide, billboardState.slideDuration);
        }
